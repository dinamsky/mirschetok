(function($){
$.siteAdvancedparamsPluginBackend = {
    default_action: '?plugin=advancedparams&action=fields&type=page',
    ...
    Field: {
        ...
        Add: function(obj) {
            ...
            var field_types = $.siteAdvancedparamsPluginBackend.field_types;
            ...
            var html = '<div class="fields-group advancedparams-field advancedparams-field-new">' +
                ...
                '<input type="hidden" name="action" value="'+$.siteAdvancedparamsPluginBackend.action+'">' +
        ...
        Save: function (obj) {
            ...
            if(post_data['action'] =='') {
                post_data['action'] = $.siteAdvancedparamsPluginBackend.action;
            }
            if(post_data['name'] =='') {
            ...
            for (var k in values) {
                if(values.hasOwnProperty(k)) {
                    ...
                    html += '<div class="advancedparams-field-value" data-id="' + value["id"] + '">' +
                        '<input type="radio"  class="advancedparams-field-value-default" name="default_value[' + value["field_id"] + ']" value="' + value["id"] + '" ' + (parseInt(value["default"]) > 0 ? " checked " : "") + '> ' +
        ...
        selectType :function (obj) {
            var field =  $(obj).closest(this.field_container_selector);
            var type = $(obj).val();
            if($.siteAdvancedparamsPluginBackend.field_types_selectable[type]) {
            ...
            if(id) {
                ...
                var success = function(response) {
                    if (response.status == 'ok') {
                        var data = response.data;
                        var html = null;
                        if($.siteAdvancedparamsPluginBackend.field_types_selectable[data.type] && typeof (data.values) =='object') {
...
$(document).ready(function () {
    ...
    $(document).off('change','.advancedparams-field-image-size input[name=size_type]').on('change','.advancedparams-field-image-size input[name=size_type]', function () {
        $.siteAdvancedparamsPluginBackend.Field.imageSizeTypeChange($(this));
    });
    $(document).off('change','.advancedparams-field-image-size input[type=text]').on('change','.advancedparams-field-image-size input[type=text]', function () {
        $.siteAdvancedparamsPluginBackend.Field.imageSizeChange($(this));
    });
    $(document).off('submit','.advancedparams-field-image-size form').on('submit','.advancedparams-field-image-size form', function () {
        $.siteAdvancedparamsPluginBackend.Field.imageSizeTypeSave($(this));
        return false;
    });
});
})(jQuery);