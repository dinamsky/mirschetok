...
$.siteAdvancedparamsPlugin = {
    ...
    Param: {
        ...
        selectFile: function (obj) {
            if($.siteAdvancedparamsPlugin.getActionId()<1) {
            ...
            var modal_form = '<div class="advancedparams_plugin-modal">' +
                ...
                '<form method="post" action="'+$.siteAdvancedparamsPlugin.file_upload_url+'" id="file-upload-'+field_name+'" enctype="multipart/form-data">' +
                '<input type="hidden" name="action" value="'+$.siteAdvancedparamsPlugin.action+'">' +
                '<input type="hidden" name="action_id" value="'+$.siteAdvancedparamsPlugin.getActionId()+'">' +
                '<input type="hidden" name="type" value="'+type+'">' +
                '<input type="hidden" name="_csrf" value="'+$.siteAdvancedparamsPlugin.getCookie("_csrf")+'">' +
                ...
                modal_form +='<input type="file" name="file" class="advancedparams_plugin-param-file-input" value="">';
                if(type == 'file') {
        ...
        fileDelete: function (obj) {
            ...
            post_data['action'] = $.siteAdvancedparamsPlugin.action;
            post_data['action_id'] = $.siteAdvancedparamsPlugin.getActionId();
            ...
            var success = function(response) {
                if (response.status == 'ok') {
                    ...
                    field.find('.advancedparams_plugin-param-file-action').html('Вставьте ссылку файла  <input type="text" name="url" class="advancedparams_plugin-param-file-input" value=""><br>или <a href="#" class="advancedparams_plugin-param-file-upload"><i class="icon16 add"></i>Загрузить</a><br><span class="errors"></span>');
                } else {
        ...
        fileUpload: function (obj) {
            ...
            var success = function(response) {
                ...
                if(typeof(response)=='object' && response.status =='ok' &&
                    ...
                    var field = $(document).find('#advancedparams_plugin-field-'+data.field_name);
                    field.find('.advancedparams_plugin-param').val(data.file_link);
            ...
            if($(obj).attr('name')=='file') {
                ...
                if($.siteAdvancedparamsPlugin.getActionId()<1) {
                    if($.siteAdvancedparamsPlugin.action == 'product') {
                ...
                $.ajax({
                    url: $.siteAdvancedparamsPlugin.file_upload_url,
                    data:  post_data,
                    type: 'post',
                    dataType: 'json',
                    success: success
        ...
        nameValid: function(obj) {
            ...
            $.ajax({
                url: $.siteAdvancedparamsPlugin.field_name_valid,
                data:  {'action':$.siteAdvancedparamsPlugin.action, 'name': name},
                type: 'post',
                dataType: 'json',
                success: success
            });
...
$(document).ready(function () {
    ...
    $(document).off('click','.advancedparams_plugin-add-param').on('click','.advancedparams_plugin-add-param', function () {
        $.siteAdvancedparamsPlugin.Param.add($(this));
        return false;
    });
    ...
    $(document).off('change, blur','.advancedparams_plugin_custom_name').on('change, blur','.advancedparams_plugin_custom_name', function () {
        $.siteAdvancedparamsPlugin.Param.nameValid($(this));
        return false;
    });
    ...
    $(document).off('dblclick','.field').on('dblclick','.field', function () {
        if($(this).find('.advancedparams_plugin-param-active')) {
            ...
            if(!ch) {
                ...
                $.siteAdvancedparamsPlugin.Param.setActive($(this).find('.advancedparams_plugin-param-active'));
                return false;
            }
        }
    });
       ...
       if($(this).val().trim() == '' || ( $(this).attr('type')=='textarea' && $(this).val()=='<p>&#8203;</p>')  ) {
          var active =  $(this).closest('.field').find('.advancedparams_plugin-param-active');
           if(active.prop('checked')) {
               ...
               $.siteAdvancedparamsPlugin.Param.setActive(active);
           }
       }  
       return false;
    });
