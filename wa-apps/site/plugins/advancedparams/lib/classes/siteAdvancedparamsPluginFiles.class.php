...
/**
 * Class siteAdvancedparamsPluginFiles
...
class siteAdvancedparamsPluginFiles {
    ...
    /**
     * siteAdvancedparamsPluginFiles constructor.
     * @param string $action
     * @param int $action_id
     */
    ...
    public function getDataPath() {
        if(empty($this->data_path)) {
            $this->data_path = wa()->getDataPath($this->getActionPath(), true, siteAdvancedparamsPlugin::APP);
        }
    ...
    public function getDataUrl() {
        if(empty($this->data_url)) {
            $this->data_url =  wa()->getDataUrl($this->getActionPath(), true, siteAdvancedparamsPlugin::APP);
        }
    ...
    protected function getActionPath() {
       return $this->getActionRootPath().siteAdvancedparamsPlugin::PLUGIN_ID."/";
    }
    ...
    public static function file_exists($url = '') {
        $real_path =  wa(siteAdvancedparamsPlugin::APP)->getConfig()->getRootPath().$url;
        ...
        foreach ($files as $file) {
            ...
            if(file_exists($real_path) && is_file($real_path)) {
                try {
                    ...
                    siteAdvancedparamsPlugin::log($ex->getMessage());
                }
            } else {
            ...
            if($delete_flag) {
                ...
                $this->deleteEmptyDirectories(wa()->getDataPath($this->getActionRootPath(), true, siteAdvancedparamsPlugin::APP), $path);
            }
        }
        $this->deleteEmptyDirectories(wa()->getDataPath($this->getActionRootPath(), true, siteAdvancedparamsPlugin::APP), $path);
    }
        ...
        if($type == 'image') {
            try {
                ...
                if ($image->save($path.$file_name)) {
                    $file_link = $this->getDataUrl().$file_name;
                    $data = array(
