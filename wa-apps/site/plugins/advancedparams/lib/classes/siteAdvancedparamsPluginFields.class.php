...
/**
 * Class siteAdvancedparamsPluginFields
...
class siteAdvancedparamsPluginFields {
    ...
    /**
     * siteAdvancedparamsPluginFields constructor.
     * @param $action string тип екшена страница, продукт или категория
     */
    public function __construct($action = '')
    {
        // Устанавливаем типы полей
        $field_types = siteAdvancedparamsPlugin::getConfigParam(siteAdvancedparamsPlugin::CONFIG_FIELD_TYPES_KEY);
        ...
        $this->field_namespace = siteAdvancedparamsPlugin::PARAM_FIELD_NAME;// префикс названия полей
        $this->field_class = siteAdvancedparamsPlugin::PARAM_FIELD_CLASS; // Css класс полей
        // Модели
        $this->fields_model = new siteAdvancedparamsFieldModel(); // Модель полей
        $this->param_value_model = new siteAdvancedparamsParamValueModel($this->action); // Модель реальных значений полей
        $this->param_file_model = new siteAdvancedparamsParamFileModel($this->action); // Модель значений файловых параметров
    ...
    public function getFields($action_id, $action_params) {
        ...
        if(!empty($action_params)) {
            foreach($action_params as $name => $value) {
                if(!siteAdvancedparamsPlugin::isBannedField('page', $name)) {
    ...
    protected function getField($field, $value = null) {
        ...
        if(method_exists($this, $method)) {
            ...
            $html = '<div class="field" id="advancedparams_plugin-field-'.htmlspecialchars($field['name']).'" data-type="'. $field['type'].'">           
                    <div class="name">'.$this->getFieldName($field,$value).'</div>
                    <div class="value">'.$field_html.'<br><span class="hint">'.htmlspecialchars($field['description']).'</span>
                    </div>
                    </div>';
            return $html;
        } else {
    ...
    protected function getCustomParam($name, $value) {
        ...
        $html = '<div class="field" id="advancedparams_plugin-field-'.$id.'" data-type="custom">           
                    <div class="name">
    ...
    protected function getSelectField($field, $value)
    {
        if(isset($field['values']) && is_array($field['values'])&& !empty($field['values'])) {
            ...
            $html = 'Добавьте значения в <a href="/'.wa()->getConfig()->getBackendUrl().'/'.siteAdvancedparamsPlugin::APP.'/?action=plugins#/advancedparams/">настройках плагина</a>!';
        }
        return $html;
    }
     ...
     * @return string (HTML)
     */
    protected function getFileField($field) {
        ...
        if(!empty($value) && !siteAdvancedparamsPluginFiles::file_exists($value)) {
            $value = null;
        }
        ...
        if(!empty($value)) {
            ...
            if($field['type']=='image') {
                $file_preview = '<a href="'.$value.'" target="_blank">'.$name.'<br><br>
                <img src="'.$value.'" class="advancedparams_plugin-param-image" />
                </a>';
            } else {
                $file_preview = '<a href="'.$value.'" target="_blank">
            ...
            $html = '<input  class="'.$this->getClass().'" type="hidden" data-type="'.$this->getType($field['type']).'"  name="'.$this->getName($field['name']).'" value="'.$value.'"  disabled>
                ...
                <div class="advancedparams_plugin-param-file-action">
                Вставьте ссылку файла  <input type="text" name="url" class="advancedparams_plugin-param-file-input" value="">
    ...
    protected function getHtmlField($field, $value) {
        ...
        $params = array(
            ...
            'value' =>  $real_value,
            'id' =>$this->getClass().'-'.htmlspecialchars($field['id'])
        );
        if(is_null($value)) {
    ...
    protected function getFieldName($field, $value) {
        $html = '<label><input type="checkbox" class="advancedparams_plugin-param-active" name="'.htmlspecialchars($this->field_namespace).'_active['.htmlspecialchars($field['id']).']" value="1" '.(!is_null($value)?'checked="true"':'').'> '.htmlspecialchars($field['title']).'<br><span class="hint">{$page.'.htmlspecialchars($field['name']).'}</span></label>';
        return $html;
    }
    ...
    protected static function getEditorControl($field_name = '', array $params = array()){
        ...
        if(version_compare($version,'1.7.13.173','>')) {
            $config['imageUpload'] = '?module=pages&action=uploadimage&r=2';
        } else {
            $config['imageUpload'] =  '?module=pages&action=uploadimage&filelink=1';
        }
        ...
        $control .= '<script>$(function(){$("#' . $params['id'] . '").redactor(' .json_encode( $config) . ');'.$contenteditable.'})</script>';
        return $control;
    }
