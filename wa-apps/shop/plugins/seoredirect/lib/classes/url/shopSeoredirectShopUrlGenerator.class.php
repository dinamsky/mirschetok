...
class shopSeoredirectShopUrlGenerator
{
	public $domain_url;
	public $route_url;
	public $url_type;
	...
	public function getUrl(shopSeoredirectAutoredirect $item)
	{
		switch ($item->getType())
		{
	...
	private function getCategoryUrl($id)
	{
		...
		return wa()->getRouting()->getUrl(
			...
			array(
				'category_url' => ($this->url_type == shopSeoredirectUrlType::FLAT)
					? $category['url']
					: $category['full_url'],
			),
			true,
	...
	private function getCatUrl($id)
	{
		...
		if ($this->url_type == shopSeoredirectUrlType::FLAT)
		{
			return $category['url'];
		}
		else
	...
	private function getSeofilterUrl($filter_id, $category_id)
	{
		...
		$is_enabled = $filter->is_enabled && shopSeoredirectSeofilterFilterExtension::isAppliedToStorefrontCategory(
			$filter,
			$storefront,
			$category_id
		);
	...
	{
		if (shopSeoredirectBrands::hasBrandCategory($brand_id, $category_id))
		{
	...
	private function getCatalogreviewsUrl($category_id, $seofilter_id = null)
	{
		...
		$seofilter_plugin_helper = shopSeoredirectSeofilterPluginHelper::getInstance();
