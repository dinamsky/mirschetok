...
class shopSeoredirectShopUrlsModel extends waModel
{
	const SEOFILTER_URL_TYPE_CATEGORY_JOIN = '3';

	protected $table = 'shop_seoredirect_shop_urls';
	...
	public function addProductPageData($product, $page)
	{
		$data = array(
			'id' => $page['id'],
			'type' => shopSeoredirectType::PRODUCT_PAGE,
			'url' => $page['url'],
			'full_url' => '',
	...
	public function addSeofilterData($filter)
	{
		...
		if ($category_id === 0)
		{
			$sql = '
SELECT *
FROM `shop_seoredirect_shop_urls`
...
';
	...
	public function addBrandData($brand)
	{
		$data = array(
			'id' => $brand['id'],
			'type' => shopSeoredirectType::PRODUCTBRANDS,
			'url' => $brand['url'],
			'full_url' => '',
			'parent_id' => 0,
			'create_datetime' => date('Y-m-d H:i:s'),
	...
	public function addBrandCategoryData($brandcategory)
	{
		$data = array(
			'id' => $brandcategory['id'],
			'type' => shopSeoredirectType::PRODUCTBRANDS_CATEGORY,
	...
	public function getByUrl($url_variants, &$can_be_catalogreviews = false)
	{
		...
		if (count($url_variants) > 0)
		{
			...
			$seofilter_joined_urls = $this->select('*')
				->where('type = :type_seofilter', array('type_seofilter' => shopSeoredirectType::SEOFILTER))
		...
		$url_types = $seoredirect_domain_model->getByField(
			array(
				'domain' => $domain,
				'route' => $route['url'],
			),
	...
	private function getUrls($all_data)
	{
		...
		foreach ($split_data[shopSeoredirectType::CATEGORY] as $category)
		{
			self::setUrlType($category, shopSeoredirectUrlType::NATURAL);
			$categories = array_merge($categories, array($category), $this->getCategoryUrls($category));
		}

		foreach ($split_data[shopSeoredirectType::PRODUCT] as $product)
		{
			$p_natural = array();
			foreach ($split_data[shopSeoredirectType::CATEGORY] as $category)
			{
		...
		foreach ($split_data[shopSeoredirectType::PRODUCT_PAGE] as $product_page)
		{
			$p_pages = array();
		...
		$pages = $this->filterPage($split_data[shopSeoredirectType::PAGE]);

		$seofilters = $this->getSeofilterUrls($categories, $split_data[shopSeoredirectType::SEOFILTER]);

		$brands = $this->getBrandsUrls($split_data[shopSeoredirectType::PRODUCTBRANDS], $split_data[shopSeoredirectType::CATEGORY]);
	...
	private function getSplitData($all_data)
	{
		...
		return array(
			shopSeoredirectType::CATEGORY => $categories,
			shopSeoredirectType::PRODUCT => $products,
			shopSeoredirectType::PRODUCT_PAGE => $product_pages,
			shopSeoredirectType::PAGE => $pages,
			shopSeoredirectType::SEOFILTER => $seofilters,
			shopSeoredirectType::PRODUCTBRANDS => $brands,
		);
	}
	...
	{
		self::setUrlType($data, shopSeoredirectUrlType::MIXED);
	...
	{
		self::setUrlType($data, shopSeoredirectUrlType::FLAT);
	...
	private function getNaturalProductUrl($product_data, $category_data = null)
	{
		if ($category_data == null)
		{
			...
			self::setUrlType($product_data, shopSeoredirectUrlType::NATURAL);
		}
		else
		{
			self::setUrlType($product_data, shopSeoredirectUrlType::NATURAL);
	...
	private function getProductReviewsUrl($product_data, $reviews = 'reviews')
	{
		...
		$product_data['type'] = shopSeoredirectType::PRODUCT_REVIEWS;
	...
	private function getBrandsUrls($brands, $categoies)
	{
		...
		foreach ($brands as $key => $brand)
		{
			...
			foreach ($categoies as $category)
			{
				...
				$brand['type'] = shopSeoredirectType::PRODUCTBRANDS_CATEGORY;
	...
	private function addSeofilterCategoryData($category)
	{
		$seofilter_urls = $this
			->select('*')
			->where('type = :type', array('type' => shopSeoredirectType::SEOFILTER))
