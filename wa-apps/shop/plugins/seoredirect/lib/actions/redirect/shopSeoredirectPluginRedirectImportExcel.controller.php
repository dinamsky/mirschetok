...
class shopSeoredirectPluginRedirectImportExcelController extends waLongActionController
{
	/** @var PHPExcel */
	private $excel;
	...
	protected function init()
	{
		...
		$this->excel = $reader->load($file->tmp_name);
		...
		$dir = wa()->getTempPath('csv/upload/seoredirect/');
		$path = "{$dir}/{$file->name}";
		$file->moveTo($path);
		$this->data['file'] = $path;
	...
	{
		$reader = PHPExcel_IOFactory::createReaderForFile($this->data['file']);
		$this->excel = $reader->load($this->data['file']);
	}
	...
	protected function readRedirects()
	{
		...
		for ($i = 0; $i < $chunk_size; $i++)
		{
			...
			{
				$value = $worksheet->getCellByColumnAndRow($position_column, $this->data['offset'] + 2)->getValue();
	...
	protected function insertRedirect($redirect, $last, $key)
	{
		...
		$redirect['hash'] = $hash;
		$redirect['type'] = shopSeoredirectRedirectModel::getType($url);
	...
	protected function validateTable(PHPExcel $excel, &$errors)
	{
		$errors = array();
		
		$worksheet = $excel->getActiveSheet();
		$count_column = PHPExcel_Cell::columnIndexFromString($worksheet->getHighestDataColumn());
		
		if ($count_column != count($this->getColumns()))
		{
	...
	protected function getCountRows(PHPExcel $excel)
	{
		$worksheet = $excel->getActiveSheet();
