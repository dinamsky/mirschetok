...
class shopMassupdatingPlugin extends shopPlugin
{
	...
	public static function getFeatures($fetch_by_code = false, $parent = false)
	{
		...
		$features = $feature_model->select('`id` as `value`, `name` as `title`, `code`, `type`, `selectable`, `multiple`')->where((!$parent ? '`parent_id` IS NULL and ' : '') . '(`type` = \'text\' or `type` = \'varchar\' or `type` = \'double\' or `type` = \'color\' or `type` = \'boolean\' or `type` LIKE \'range%\' or `type` LIKE \'dimension%\' or `type` LIKE \'2d%\' or `type` LIKE \'3d%\')')->order('name ASC')->fetchAll($fetch_by_code ? 'code' : 'value');
	...
	public function getFeaturesControls($ids, $product_ids = array())
	{
		...
		uasort($features, array(__CLASS__, 'sortFeaturesControls'));
		
		return $features;
	}

	private function sortFeaturesControls($f1, $f2) {
