$.extend($.importexport.plugins, {
    facebookpro: {
        ...
        $form: null,
        init: function (data) {
            this.$form = $("#s-plugin-facebookpro");
        ...
        actionHandler: function ($el) {
            try {
                ...
                if (method.name) {
                    $.shop.trace('$.importexport.plugins.facebookpro', method);
        ...
        initForm: function () {
            ...
            this.$form.find('a.js-action').unbind('click').bind('click.facebookpro', function () {
            ...
            this.$form.unbind('submit.facebookpro').bind('submit.facebookpro', function (event) {
                $.shop.trace('submit.facebookpro ' + event.namespace, event);
                try {
                    var $form = $(this);
                    $form.find(':input, :submit').attr('disabled', false);
                    $.importexport.plugins.facebookpro.facebookproHandler(this);
                } catch (e) {
                    $('#plugin-facebookpro-transport-group').find(':input').attr('disabled', false);
                    $.shop.error('Exception: ' + e.message, e);
                }
                return false;
            });
        ...
        autocompleteFeature: function (event, ui) {
            ...
            if ($input.attr('name').match(/\[param\.[^\]]+\]\[feature\]$/)) {
                ...
                $.importexport.plugins.facebookpro.setParamName($div, this.value, unit[1] || '');
            }
        ...
        paramAdd: function (el, type_id) {
            ...
            $.tmpl('facebookpro-' + type_id, {id: ++this.data.params[type_id]}).insertBefore($target);
            ...
            $scope.find('a.js-action').unbind('click').bind('click.facebookpro', function () {
                return self.actionHandler($(this));
            });
        ...
        facebookproHandler: function (element) {
            var self = this;
            self.progress = true;
            self.form = $(element);
        ...
        progressHandler: function (url, processId, response) {
            ...
            var self = $.importexport.plugins.facebookpro;
            var $bar;
            if (response && response.ready) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $("#plugin-facebookpro-submit").hide();
                        self.form.find('.progressbar').hide();
                        var $report = $("#plugin-facebookpro-report");
                        $report.show();
                        if (response.report) {
