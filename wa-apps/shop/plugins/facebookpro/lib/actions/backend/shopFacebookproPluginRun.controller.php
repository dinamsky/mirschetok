<?php
class shopFacebookproPluginRunController extends waLongActionController
{
    ...
    protected function init()
    {
        try {
            ...
            $profiles = new shopImportexportHelper('facebookpro');
            ...
            if ($backend) {
                $this->data['min_price'] = waRequest::post('min_price');
                ...
                $profile_config = array(
                    ...
                    //'min_price' => waRequest::post('min_price'),
                );
...
XML;
            //TODO: prepate correct shops.dtd
            //waFiles::copy(shopFacebookproPlugin::path('shops.dtd'), $this->getTempPath('shops.dtd'));
            ...
            $this->dom->lastChild->appendChild($shop = $this->dom->createElement("channel"));
           // $name1 = $config->getGeneralSettings('name');
           ...
           // $minprice = $profile_config['export']['min_price'];
           // $minprice = $this->data['export']['min_price'];
          


            $this->addDomValue($shop, 'name', $name);

           // $this->addDomValue($shop, 'min', $minprice);
          

          $this->addDomValue($shop, 'description', $company);
            $this->addDomValue($shop, 'url', preg_replace('@^https@', 'http', wa()->getRouteUrl('shop/frontend', array(), true)));
               ...
               // $shop->appendChild($this->dom->createElement('min_price', $minprice));
            ...
            //$shop->appendChild($this->dom->createElement('offers1'));
            //$shop->appendChild($this->dom->createElement('!--1 --'));
            ...
            $this->data['path'] = array(
                'offers' => shopFacebookproPlugin::path($profile_id.'.xml'),
            );
    ...
    /**
     * @uses shopFacebookproPluginRunController::stepCategory()
     * @uses shopFacebookproPluginRunController::stepProduct()
     */
    ...
    protected function finish($filename) // сформированный файл
    {
        $result = !!$this->getRequest()->post('cleanup');
        try {
            if ($result) {
                $this->logAction('catalog_export', array('type' => 'AML',));
                $file = $this->getTempPath(); // путь до сформированного файла
                $target = $this->data['path']['offers']; //старый файл
                if (file_exists($file)) { //если новый файл существует, то
                    waFiles::delete($target); //удаляем старый
                    waFiles::copy($file, $target); //подменяем новым
                    if (file_exists($target)) { //если появился новый файл
                        waFiles::delete($file); //то удаляем временный
                    }
                }
                //TODO: prepare correct shops.dtd for validation
                //$this->validate();
            }
        } catch (Exception $ex) {
            $this->error($ex->getMessage());
    ...
    private function validate()
    {
        ...
        shopFacebookproPlugin::path('shops.dtd');
        ...
        if ((!$valid || $strict) && ($r = libxml_get_errors())) {
            ...
            if ($valid) {
                $this->data['error'][] = array(
                    'level'   => 'info',
                    'message' => 'XML-файл валиден',
                );
            } else {

                $this->data['error'][] = array(
                    'level'   => 'error',
                    'message' => 'XML-файл содержит ошибки',
                );
            }
            foreach ($r as $er) {
                /**
    ...
    private function plugin()
    {
        static $plugin;
        if (!$plugin) {
            $plugin = wa()->getPlugin('facebookpro');
        }
        return $plugin;
    }
     ...
     *
     * @return bool
     * @usedby shopFacebookproPluginRunController::step()
     */
    ...
    private function getTempPath($file = null)
    {
        ...
        return wa()->getTempPath('plugins/facebookpro/', 'shop').$file;
    }
    ...
    public function getAllCategories($id, $c){
        $model = new waModel();
        $r = $model->query("SELECT scp.category_id 
                           FROM shop_category_products scp 
                           WHERE scp.product_id = '$id'")->fetchAll();
        $allCategories = array();
        ...
        foreach($r as $key => $val){
            $allCategories[$key] = $val['category_id'];

            if($val['category_id'] == $c){
                $currKey = $key;
            }

        }

        unset($allCategories[$currKey]);
        array_unshift($allCategories, $c);

        return $allCategories;
    }

    public function getTreee($id, $c = true, $ow = 0)
    {
        ...
        if($step['parent_id'] != (int)0)
        {
            $this->getTreee((int)$step['parent_id'], $c = false);
        }
    ...
    public function chainCat($productId, $c){

        /*$cate = $this->getAllCategories($productId, $c);
        $ti = array();
        $_POST['ti'] = $ti;

        foreach($cate as $key=>$val){
            $_POST['c'][] = array_reverse($this->getTreee((int)$val));
            $_POST['ti'] = '';
        }*/
	...
	private function currentCurrency($productCurrency, $price, $comparePrice){
		$exportCurrency = $this->data['export']['cur'];
		//$primaryCurrency = $this->data['primary_currency'];

		$prices = array();                              //  Массив который будем возвращать
		$model = new shopCurrencyModel();               //  Модель валют
		$currencyArray = $model->getCurrencies();       // Все доступные валюты

		if($exportCurrency !== $productCurrency && $exportCurrency !== Null){

			$convertPrice = $model->convert($price, $productCurrency, $exportCurrency);
			$convertComparePrice = $model->convert($comparePrice, $productCurrency, $exportCurrency);
			if(!empty($this->data['export']['round']))
			{
				$prices['price']        = shopRounding::round($convertPrice, $currencyArray[$exportCurrency]['rounding'], $currencyArray[$exportCurrency]['round_up_only']);
				$prices['comparePrice'] = shopRounding::round($convertComparePrice, $currencyArray[$exportCurrency]['rounding'], $currencyArray[$exportCurrency]['round_up_only']);
			}else{
				$prices['price']        = $convertPrice;
				$prices['comparePrice'] = $convertComparePrice;
			}

		}else{
			if(!empty($this->data['export']['round']))
			{
				$prices['price']        = shopRounding::round($price, $currencyArray[$exportCurrency]['rounding'], $currencyArray[$exportCurrency]['round_up_only']);
				$prices['comparePrice'] = shopRounding::round($comparePrice, $currencyArray[$exportCurrency]['rounding'], $currencyArray[$exportCurrency]['round_up_only']);
			}else{
				$prices['price']        = $price;
	...
	private function stepProduct(&$current_stage, &$count, &$processed)
    {
        ...
        if (!$products)
        {
            ...
            {
                /*$sku_model = new shopCategoryModel();
        ...
        while ((--$chunk > 0) && ($product = reset($products))) {
            ...
            //Get min_price from backend 
            if(!empty($this->data['export']['min_price'])){
                    $min_price = (int)$this->data['export']['min_price'];
            }else{
                  $min_price = 0.5;  
            };

								shopFacebookproPluginRunController::$more2countvariation = false;
								shopFacebookproPluginRunController::$skulink = null;
								$sku_model = new shopProductSkusModel();
								$check_skus_count = $sku_model->getDataByProductId($product['id']);
								$countsku = 0;
								$valskuprice = null;
								$skus_list_price = array();
								if ($this->data['export']['id_good']) {
									shopFacebookproPluginRunController::$itemgroupid = $check_skus_count[key($check_skus_count)]['sku'];
								} else {
									shopFacebookproPluginRunController::$itemgroupid = $product['id'];
								}
								if ($this->data['export']['ms1c_goods']) { // Использовать ли 1с или мой склад контейнер. Настройки плагина(чекбокс)
									$check_use_1c = new shopFacebookproCheckUse1C($product, $check_skus_count);
									$isUseProduct1c = $check_use_1c->checkUse1Cproduct(); // проверяем использует ли данный товар 1С систему
								}
								foreach ($check_skus_count as $keysku => $valuesku) {
									if ($valuesku[price]!==$valskuprice) {
												$countsku++;
												$skus_list_price[$valskuprice] = $valskuprice = $valuesku[price];
												$maxskuprice = max($skus_list_price);
										}
									if ($countsku>=2) {
										$product['price'] = $maxskuprice;//Если это вариации с разными ценами, то устанавливаем самую высокую цену
										shopFacebookproPluginRunController::$more2countvariation = true;
									}
								}

            $check_price = $product['price'] >= $min_price; //сравнивает цену товара с ценой минималки

            $check_category = !empty($product['category_id']);
            if ($check_type && $check_price && $check_category) { //проверяет все ли тру, чтобы обработать товар
                if (empty($sku_model)) {
                    $sku_model = new shopProductSkusModel();
                }
                $skus = $sku_model->getDataByProductId($product['id']); // подтягивает массив вариаций включаяя 1с индификатор, каждой вариации $skus['идВариации']['id_1c']
               ...
               // $this->chainCat((int)$product['id'], (int)$product['category_id']);
                $this->getTreee((int)$product['category_id']);

                $product['cate'] = $_POST['c'];
	            ...
	            $options = array(
		            ...
		            $backend_product = new shopProductsCollection('id/'. $product['id'], $options);
	          
				$product_backend_price = array_shift($backend_product->getProducts());
                foreach ($skus as $sku) {
                	$curSku = $sku_model->getPrices($sku['id']); // Цена вариации товара
									$isItVariationOrNot = true;
									if ($this->data['export']['ms1c_goods']) {
										$check1Ctype = new shopFacebookproCheck1CContainer($product['id_1c'], $sku['id_1c']);
										$isItVariationOrNot = $check1Ctype->isItVariation();
									}

										$compare_variation_minprice = $sku['price'] >= $min_price;

                    $model = new shopCurrencyModel();
                    $currencyArray = $model->getCurrencies();

                    // Получаем валюту для витрины

									if ($compare_variation_minprice && ($check_stock || ($sku['count'] === null) || ($sku['count'] > 0)) && $isItVariationOrNot) { // Также проверяем, контейнер 1с или вариация, если вариация, то выполняем, если контейнер, то пропускаем
										if (empty($skuPrices[$sku['primary_price']])) {

	                        $prices = $this->currentCurrency($product_backend_price['currency'], $sku['price'], $sku['compare_price']);
	                        $product['price'] = array(
		                        'price'         => $prices['price'],
		                        'compare_price' => $prices['comparePrice']
	                        );

                            $product['compare_price'] = $sku['compare_price'];
                            $skuPrices[$sku['price']] = array(
                                'product' => $product,
                                'skus' => array(),
                            );
                        }
                        		$skuPrices[$sku['price']]['skus'][] = $sku; //Присваиваем массив данных вариации карточке товара.
                    }
                }

                shopFacebookproPluginRunController::$count_variation_interation = $count_variation_interation = count($skuPrices, COUNT_NORMAL) - 1; //колличество вариаций(интераций для объекта класса вhopFacebookproAddVariationIDorArticulNumber)
                foreach ($skuPrices as $skuPrice) {
                    $product = $skuPrice['product']; // Добавляем в продукт массив собранной информации
									if (shopFacebookproPluginRunController::$more2countvariation && $this->data['export']['variation_name_plus_articul']){ // Меняем название вариации - добавляем наименование артикула
										$product['name'] = $product['name'] . ' ' . $skuPrice[skus][0][name];
									}
                    // меняем поле артикул - задаем в данное поле артикул
                    $product['sku'] = $skuPrice['skus'][0]['sku'];
                    $skus = $skuPrice['skus']; //Вариация перезаписывается в 0 индекс
                    $this->addOffer($product, ifempty($this->data['types'][$product['type_id']], 'simple'), $skus, $skus[0]['sku']);
										++$processed;
                }
            }
...
//идет интерация каждой вариации
//максимальную сравнить с минимальной и обратиться к созданному объекту, на итих услових строить вывод
		public static $more2countvariation;
		public static $itemgroupid;
		public static $count_variation_interation;
		public static $count_variation;
		public static $utm_link_variation;
		public static $skulink;
    private function addOffer($product, $type, $skus = null, $kod = null)
    {
        static $features;
        ...
        if ($product["min_price"] !== $product["max_price"]) {
				$kodvariation = new shopFacebookproAddVariationIDorArticulNumber();
				$kod = $kodvariation->idOrArtVariation($product['id'], $skus[0]['id'], $product['id_1c'], $skus[0]["id_1c"], $this->data['export']['id_good'], $this->data['export']['ms1c_goods'], $kod);
				}  elseif (empty($this->data['export']['id_good'])) {
					$kod = $product['id'];
				}
           ...
           $nodes = $this->dom->getElementsByTagName('channel');

            $offers = $nodes->item(0);
        }

        $product_xml = $this->dom->createElement("item");
        $offer_map = $this->data['map'][$type]['fields'];
        $variantList = array();

			if(!empty($kod)) {
				$e = $this->dom->createElement('g:id', $kod);
			} else {
				$e = $this->dom->createElement('g:id', 'Заполните код артикула');
			}

        $product_xml->appendChild($e);
        //$product_xml->appendChild($pro);
        foreach ($offer_map as $field => $info) {
            ...
            if (!empty($info['source']) && (!ifempty($info['category'], array()) || in_array('simple', $info['category']))) {
                ...
                switch ($source) {
                    case 'field':
                        //check and change price
                        $value = ifset($product[$param]);
                        $value = $this->format($field, $value, $info, $product);
                        break;
                    case 'value':
                    case 'text':
                    ...
                    case 'feature':
                        if (!isset($product['features'])) {
                            ...
                            $product['sku_features'] = $product_features_model->getSkuFeatures($product['id']);
                        }
                        if (!isset($features)) {
                            $features_model = new shopFeatureModel();
                            $features = $features_model->getFeatures(true, null, 'code');
                        }
                        $values = $value = $this->format($field, ifempty($product['features'][$param]), $info);
                        break;
                }
                if ($field == 'variant') {
                    foreach ($skus as $sku) {
                        // если данное значение берётся из множесвенной характиристики, то берём вариант для данного sku
                        if($source == 'feature' && $features[$param]['multiple']){
                            $feature_id = $features[$param]['id'];
                            $feature_value_id = @$product['sku_features'][$sku['id']][$feature_id];
                            if($feature_value_id){
                                $value = trim(strip_tags($this->format($field, $product['features'][$param][$feature_value_id], $info)));
                            }else $value = '';
                        }
                        if (empty($variantList[$sku['id']])) {
                            $variantList[$sku['id']] = array(
                                'variant' => array(
                                    'offerId' => $sku['id'],
                                ),
                            );
                        }
                        $variant = &$variantList[$sku['id']];
                        if ($source == 'field') {
                            switch ($param) {
                                case 'id':
                                    $value = $sku['id'];
                    ...
                    $this->addDomValue($product_xml, $field, $value);
        ...
        if( !empty($this->data['export']['category'])  && empty($product['features']['g_product_type']) )
        {
            if (!empty($product['cate']))
            {
                $full_name = '';
                $reverse   = array_reverse($product['cate']);

                    foreach ($reverse as $key1 => $val1)
                    {
                        if ($key1 != 0)
                            $full_name .= ' > ' . $val1['name'];
                        else
               ...
               $this->addDomValue($product_xml, 'g:product_type', $full_name);
            }
        }
	    $this->addDomValue($product_xml, 'g:adult', $this->data['export']['audit']);
        if (!empty($variantList)) {
            // если в размере только атрбуты, а самого размера нет, то удаляем размер [begin]
            unset($variant);
            foreach ($variantList as $variant_id => $variant) {
                $variant = $variant['variant'];
                if (!isset($variant['size'])) continue;
                $size = $variant['size'];
                if(!$size || is_array($size) && !$size['value']){
                    unset($variantList[$variant_id]['variant']['size']);
                    if(!isset($variant['color'])) unset($variantList[$variant_id]);
                }
            }
            unset($i);
            // если в размере только атрбуты, а самого размера нет, то удаляем размер [end]
            // убираем варианты, где ни цвета, ни размера
            unset($variant);
            foreach ($variantList as $variant_id => $variant) {
                $variant = $variant['variant'];
                if (empty($variant['size']) && empty($variant['color'])){
                    unset($variantList[$variant_id]);
                }
            }
        }

        if (!empty($variantList)) {
            // если размер в варианте содержит /, то разбиваем на несколько вариантов [begin]
            $alfa = array('a', 'b', 'c', 'd', 'e');
            unset($variant);
            foreach ($variantList as $variant_id => $variant) {
                $variant = $variant['variant'];
                if (!empty($variant['size'])){
                    if(is_array($variant['size'])){
                        $sizes = explode('/', $variant['size']['value']);
                        if(count($sizes) > 1){
                            unset($variantList[$variant_id]);
                            $i = 0;
                            foreach($sizes as $size){
                                $offerId = $variant_id.$alfa[$i];
                                $variantList[$offerId] = array('variant' => array('offerId' => $offerId, 'size' => array('value' => $size) + $variant['size']) + $variant);
                                $i++;
                            }
                        }
                    }else{
                        $sizes = explode('/', $variant['size']);
                        if(count($sizes) > 1){
                            unset($variantList[$variant_id]);
                            $i = 0;
                            foreach($sizes as $size){
                                $offerId = $variant_id.$alfa[$i];
                                $variantList[$offerId] = array('variant' => array('offerId' => $offerId, 'size' => $size) + $variant);
                                $i++;
                            }
                        }
                    }
            ...
            unset($i);
            // если размер в варианте содержит /, то разбиваем на несколько вариантов [end]

            $variantList_xml = $this->dom->createElement("variantList");
            unset($variant);
            foreach ($variantList as $variant) {
                $variant = $variant['variant'];
                $variant_xml = $this->dom->createElement("variant");
                if (!empty($variant['size']) && !empty($variant['color']))
                    $variant_xml->setAttribute('type', 'color_and_size');
                elseif (!empty($variant['size']))
                    $variant_xml->setAttribute('type', 'size');
                elseif (!empty($variant['color']))
                    $variant_xml->setAttribute('type', 'color');
                foreach ($variant as $key => $value)
                    $this->addDomValue($variant_xml, $key, $value);
                $variantList_xml->appendChild($variant_xml);
            }
            $product_xml->appendChild($variantList_xml);
        }
       // $this->dom->appendChild($product_xml);
		...
		if (is_array($value)) {
            ...
            if (key($value) !== 0) {
                ...
                elseif($field == 'availability'){
                    $dom->appendChild($this->dom->createElement('g:availability', $value['stock']));
                    $dom->appendChild($this->dom->createElement('g:expiration_date', $value['date']));
                }
                elseif($field == 'price'){
                    if($value['compare_price'] != 0){
                        $dom->appendChild($this->dom->createElement('g:price', $value['compare_price'] .' '. $this->data['export']['cur']));
                        $dom->appendChild($this->dom->createElement('g:sale_price', $value['price'] .' '. $this->data['export']['cur']));
                    }else{
                        $dom->appendChild($this->dom->createElement('g:price', $value['price'] .' '. $this->data['export']['cur']));
                    }
                ...
                if($field == 'picture' &&  !isset($this->data['export']['custom_img']) ){
                    $count = 1;
                    foreach ($value as $value_item) {
                       //$dom->appendChild($this->dom->createElement($field, trim($value_item)));
                            
                        if($count == 1){
                            $dom->appendChild($this->dom->createElement('g:image_link', trim($value_item)));
                            $count++;
                        }else{
                            $dom->appendChild($this->dom->createElement('g:additional_image_link', trim($value_item)));
                        }
                    }
                }
             ...
             /*elseif($field == 'color'){       
                    foreach ($value as $value_item) {               
                            $dom->appendChild($this->dom->createElement('g:color', trim($value_item))); 
                    }
                }*/
            ...
            if(!in_array($field, $ignore_fields) && $value != NULL){
                ...
                if ($field == 'categoryId') {
                 ...
                 elseif($field == 'offer'){
                    $dom->appendChild($this->dom->createElement('item', $value)); 
                }
                
                 elseif($field == 'url'){
                	if (shopFacebookproPluginRunController::$more2countvariation) {
										$dom->appendChild($this->dom->createElement('g:item_group_id', shopFacebookproPluginRunController::$itemgroupid));
									}
                    $dom->appendChild($this->dom->createElement('link', $value)); 
                }
                elseif($field == 'title'){
                    $dom->appendChild($this->dom->createElement('title', $value)); 
                }
                elseif($field == 'brand'){
                    $dom->appendChild($this->dom->createElement('g:brand', $value)); 
                }

                elseif($field == 'custom_label_0'){
                    $dom->appendChild($this->dom->createElement('g:custom_label_0', $value));
                }
                elseif($field == 'custom_label_1'){
                    $dom->appendChild($this->dom->createElement('g:custom_label_1', $value));
                }
                elseif($field == 'custom_label_2'){
                    $dom->appendChild($this->dom->createElement('g:custom_label_2', $value));
                }
                elseif($field == 'custom_label_3'){
                    $dom->appendChild($this->dom->createElement('g:custom_label_3', $value));
                }
                elseif($field == 'GTIN'){
                    if($value != 'no_gtin'){
                      $dom->appendChild($this->dom->createElement('g:gtin', $value));
                    }else{
                        $dom->appendChild($this->dom->createElement('g:identifier_exists', 'no'));
                    }
                }

                 elseif($field == 'g:product_type'){
                    $dom->appendChild($this->dom->createElement('g:product_type',$value));
                }   

                elseif($field == 'g:google_product_category'){
                    $dom->appendChild($this->dom->createElement('g:google_product_category',$value));
                }
                elseif($field == 'mpn'){
                    if(!empty($value)){
                      $dom->appendChild($this->dom->createElement('g:mpn', $value));
                    }
                }
								elseif($field == 'currencyId'){
									$dom->appendChild($this->dom->createElement($f, '&#110;&#101;&#119;'));
								}
                elseif($field == 'id'){
                    //$dom->appendChild($this->dom->createElement('g:ttt', 'ttt'));
                }
                elseif($field == 'cate'){
                    $dom->appendChild($this->dom->createElement('g:cate', $value));
                }

                elseif($field == 'custom_img'){
                    if(isset($this->data['export']['custom_img']) ){
                        $dom->appendChild($this->dom->createElement('g:image_link',  trim($value)));
                    }
                }
                else{
    ...
    private function format($field, $value, $info = array(), $data = array(), $sku_data = null)
    {
        ...
        switch ($field) {
            ...
            case 'availability':
                ...
                if(!empty($this->data['export']['strict_goods'])){
            ...
            case 'url':
                //max 512
                $value = preg_replace_callback('@([^\[\]a-zA-Z\d_/-\?=%&,\.]+)@i', array(__CLASS__, '_rawurlencode'), $value); //В data находится ИД товара и вариации, все что нужно для формирования дополнительной информации в utm метки. Формирование ссылки link в товаре.
                if ($this->data['utm']) {
										$proutm = '&utm_idproduct='.$data['id'].'&utm_mainarticulproduct='.$data['sku'];
										if ($data[min_price]!==$data[max_price]){
											$proutm .= shopFacebookproPluginRunController::$utm_link_variation;
										}
                    $value .= (strpos($value, '?') ? '&' : '?').shopFacebookproPluginRunController::$skulink.$this->data['utm'].$proutm;
                }
                $value = 'https://'.ifempty($this->data['base_url'], 'localhost').$value;
            ...
            case 'picture':
                ...
                while (is_array($value) && ($image = array_shift($value)) && $limit--) {
                    ...
                    $values[] = 'https://'.ifempty($this->data['base_url'], 'localhost').shopImage::getUrl($image, $size);
                }
                $value = $values;
                break;
            case 'page_extent':
        ...
        if (is_array($value)) { // Возможно проверяет, просто одна вариация - карточка товара или массив в карточке несколько вариаций.
            /**
             * @var $value array
             */
            reset($value);
    ...
    private function error($message)
    {
        ...
        waFiles::create($path.'/shop/plugins/facebookpro.log');
        waLog::log($message, 'shop/plugins/facebookpro.log');
    }
