...
class shopFacebookproPluginFrontendActions extends waActions
{
    public function catalogAction()
    {
        /**
         * @var shopFacebookproPlugin $plugin
         */
        $plugin = wa()->getPlugin('facebookpro');

        $profile_helper = new shopImportexportHelper('facebookpro');
        ...
        if ($profile_id) {
            ...
            if ($lifetime && (!file_exists($path) || (time() - filemtime($path) > $lifetime))) {
                ...
                $runner = new shopFacebookproPluginRunController();
        ...
        waFiles::readFile($path, waRequest::get('download') ? 'facebookpro.xml' : null);
    }
}
