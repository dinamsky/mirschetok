...
class shopEditpricePluginSkusController extends waJsonController
{
    public function execute()
    {
        ...
        $this->response['stocks'] = $stock_model->select('id,name')->order('sort ASC')->fetchAll();
        ...
        if ($id) {
            ...
            $sku_stocks = array();
            if ($plugin->getSettings('stock')) {
                $product_stock_model = new shopProductStocksModel();
                $rows = $product_stock_model->getByField('product_id', $id, true);
                foreach ($rows as $row) {
                    $sku_stocks[$row['sku_id']][(string)$row['stock_id']] = $row['count'];
                }
            }
            ...
            foreach ($rows as $row) {
                ...
                if (isset($sku_stocks[$row['id']])) {
                    $row['stock'] = $sku_stocks[$row['id']];
                }
                $skus[$product_id][] = $row;
            }
