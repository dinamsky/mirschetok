...
class shopEditPricePluginSaveController extends waJsonController
{
    ...
    public function execute()
    {
         ...
         */
        $plugin = wa('shop')->getPlugin('editprice');
        $edit_stock = $plugin->getSettings('stock');
        ...
        foreach ($rows as $row) {
            ...
            if (isset($old_data[$row['product_id']][$row['sku_id']]) && isset($stocks[$row['stock_id']])) {
                $old_data[$row['product_id']][$row['sku_id']]['stock'][$row['stock_id']] = $row['count'];
            }
        }
        ...
        foreach ($data as $product_id => $product_data) {
            ...
            foreach ($product_data as $sku_id => $sku) {
                ...
                if ($edit_stock) {
                    if (array_key_exists('count', $sku)) {
                        ...
                        if ($old_data[$product_id][$sku_id]['available'] || !empty($update['available'])) {
                            $count = $this->addSkuCount($count, $sku['count']);
                        }
                    } else {
                        $sku_count = 0;
                        foreach ($stocks as $stock_id => $s) {
                            $stock_count = ifset($sku['count_'.$stock_id], '');
                            if (trim($stock_count) === '') {
                                $stock_count = null;
                            }

                            if ($stock_count !== ifset($old_data[$product_id][$sku_id]['stock'][$stock_id])) {
                                $is_changed = true;
                                $update['stock'][$stock_id] = $stock_count;
                            }
                            if ($sku_count !== null && $stock_count !== null) {
                                $sku_count += $stock_count;
                            } else {
                                $sku_count = null;
                            }
                        }
                        if ($sku_count !== $old_data[$product_id][$sku_id]['count']) {
                            $is_changed = true;
                            $update['count'] = $sku_count;
                        }
                        if ($old_data[$product_id][$sku_id]['available'] || !empty($update['available'])) {
                            $count = $this->addSkuCount($count, $sku_count);
                        }
                    }
                }
                ...
                if ($update) {
                    ...
                    // update stock
                    if (array_key_exists('stock', $update)) {
                        foreach ($update['stock'] as $stock_id => $stock_count) {
                            $where = array('sku_id' => $sku_id, 'stock_id' => $stock_id);
                            if ($stock_count === null) {
                                $product_stocks_model->deleteByField($where);
                            } elseif (isset($old_data[$product_id][$sku_id]['stock'][$stock_id])) {
                                $product_stocks_model->updateByField($where, array('count' => $stock_count));
                            } else {
                                $product_stocks_model->insert(array(
                            ...
                            $stocks_log_model->add(array(
                                ...
                                'sku_id' => $sku_id,
                                'stock_id' => $stock_id,
                                'before_count' => ifset($old_data[$product_id][$sku_id]['stock'][$stock_id]),
                                'after_count' => $stock_count,
                            ));
                        }
                    } elseif (array_key_exists('count', $update)) {
                        if (!empty($old_data[$product_id][$sku_id]['stock']) && count($old_data[$product_id][$sku_id]['stock']) == 1) {
                            $stock_id = key($old_data[$product_id][$sku_id]['stock']);
                            $where = array('sku_id' => $sku_id, 'stock_id' => $stock_id);
    ...
    protected function addSkuCount($count, $sku_count)
    {
        if ($sku_count !== null && ($sku_count < 0)) {
            return $count;
        }
        if ($count === false) {
            $count = $sku_count;
        } elseif ($sku_count !== null && $count !== null) {
            $count += $sku_count;
        } else {
            $count = null;
        }
