...
class shopDelpayfilterHelper
{
    ...
    {
        // JS Локализция
        $js_locale_path = wa()->getAppPath('plugins/delpayfilter/locale/' . wa()->getLocale() . '/LC_MESSAGES/shop_delpayfilter_js_backend.json', 'shop');
        $js_locale_strings = [];
    ...
    /**
     * Order total price without shipping price
     *
     * @param bool $discount
     * @return float
     * @throws waException
    ...
    public function getTotalPrice($discount = true)
    {
        ...
        if (waRequest::param('plugin', '') == 'quickorder') {
            ...
            $total = $order_params['quickorder_cart']->getTotal(false, $discount);
        } else {
            $total = (new shopCart())->total($discount);
        }
        return (float) shop_currency($total, wa('shop')->getConfig()->getCurrency(false), wa('shop')->getConfig()->getCurrency(true), false);
    }
    ...
    public function getTotalDiscount()
    {
        ...
        if (waRequest::param('plugin', '') == 'quickorder') {
            $order_params = shopDelpayfilterData::getCurrentOrderParams();
            $discount = $order_params['quickorder_cart']->getDiscount();
        } else {
            $discount = (new shopCart())->discount();
        }
        return (float) shop_currency($discount, wa('shop')->getConfig()->getCurrency(false), wa('shop')->getConfig()->getCurrency(true), false);
    }
}
