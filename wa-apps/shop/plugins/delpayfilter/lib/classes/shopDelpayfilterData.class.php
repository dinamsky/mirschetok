...
class shopDelpayfilterData
{
    ...
    private static $typesMethods = array(
        ...
        'product_stock_storefront' => 'stocks',
        'user_data' => 'userData2',
    );
        ...
        if (isset(self::$category_tree[$category_id])) {
            ...
            if (!self::$category_tree[$category_id]['type']) {
                ...
                if (!isset(self::$dynamic_products[$category_id])) {
                    ...
                    $pc = new shopProductsCollection('category/' . $category_id, ['no_plugins' => 1]);
    ...
    public static function getSetProducts($set_id)
    {
        if (!isset(self::$sets[$set_id])) {
            $pc = new shopProductsCollection('set/' . $set_id, ['no_plugins' => 1]);
    ...
    {
        $addresses = array();
        $contact = (new shopDelpayfilterHelper())->getContact();
        if ($contact) {
    ...
    {
        $hash = 'id/' . implode(",", $ids);
        $collection = new shopProductsCollection($hash, ['no_plugins' => 1]);
    ...
    public static function getCurrentOrderParams()
    {
        ...
        if (isset($params['order'])) {
            ...
            if ($shipping) {
                $parts = explode('.', $params['shipping']['variant_id'], 2);
                $params['shipping']['id'] = $parts[0];
                $params['shipping']['rate_id'] = $parts[1];
            }
        }
        if (empty($params['shipping']['id'])) {
            $params['shipping'] = array('id' => 0);
        }
        $params['contact'] = (new shopDelpayfilterHelper())->getContact();
    ...
    public function getCouponCodes()
    {
        if (shopDiscounts::isEnabled('coupons')) {
            $data = self::getCurrentOrderParams();
        }
