...
$.flexdiscount = {
    ...
    initSettingsPage: function (options) {
        ...
        /* Сохранение настроек */
        $("#s-flexdiscount-form").submit(function () {
            that.saveSettingsHandler($(this));
            return false;
        });
        ...
        $('#s-flexdiscount-form .js-show-more-plugins').click(function () {
            $(this).siblings('.plugin-list').find('li').slideDown();
            $(this).remove();
        });

        /* Выбор способа вывода стилей */
        $(".js-styles-output input").change(function () {
            if ($(this).val() == 'helper') {
    ...
    saveSettingsHandler: function (form) {
        if (!this.hasLoading($("#fixed-save-panel"))) {
            this.appendLoading($("#fixed-save-panel .block"));
            $.post("?plugin=flexdiscount&module=settings&action=save", form.serializeArray(), function (response) {
                ...
                $.flexdiscount.removeLoading($("#fixed-save-panel"));
                setTimeout(function () {
        ...
        if (detailsSelect.data('chosen') === undefined) {
            detailsSelect.show().chosen({disable_search_threshold: 10, no_results_text: $__("No result text")})
                ...
                if (!detailsBlock.hasClass('s-detail-' + value)) {
                    ...
                    } else if (value == 'ncheapest') {
                        html += $__("every") + " <input name='details' type='text' onkeypress='$.flexdiscount.isValid(event, /[0-9]/);' class='width50px'> " + $__('th cheapest products');
                    ...
                    } else if (value == 'nexpensive') {
                        html += $__("every") + " <input name='details' type='text' onkeypress='$.flexdiscount.isValid(event, /[0-9]/);' class='width50px'> " + $__('th the most expensive products');
    ...
    systemPopupSettingsAction: function () {
        var selector = 'settings-flexdiscount-dialog';
        var dialogParams = {
            loading_header: $__("Wait, please..."),
            'min-height': '270px',
    ...
    settingsCustomizeBlockAction: function (btn) {
        var type = btn.data("type");
        var dialogParams = {
            loading_header: $__("Wait, please..."),
            class: 'condition-dialog',
            ...
            onSubmit: function (form) {
                ...
                if (!$.flexdiscount.hasLoading(buttonBlock)) {

                    waEditorUpdateSource({'id': 'template-textarea'});

                    form.find('.errormsg').html('');
                    $.flexdiscount.appendLoading(buttonBlock);
                    $.post("?plugin=flexdiscount&action=handler", {
                        data: 'editSettings',
