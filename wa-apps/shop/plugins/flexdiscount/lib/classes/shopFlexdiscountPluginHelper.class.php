...
class shopFlexdiscountPluginHelper
{
    ...
    public static function getUserDiscounts($view_type = null)
    {
        ...
        if (shopDiscounts::isEnabled('flexdiscount')) {
            ...
            if (!empty($settings['user_discounts'])) {
                ...
                $html .= "<div class='flexdiscount-user-discounts' data-view-type='" . ($view_type ? shopFlexdiscountHelper::escape($view_type) : '0') . "'>";
    ...
    public static function getUserAffiliate($view_type = null)
    {
        ...
        if (shopDiscounts::isEnabled('flexdiscount') && shopAffiliate::isEnabled()) {
            ...
            if (!empty($settings['affiliate_block'])) {
                ...
                $html .= "<div class='flexdiscount-user-affiliate wa-order-bonus' data-view-type='" . ($view_type ? shopFlexdiscountHelper::escape($view_type) : '0') . "'>";
                waSystem::pushActivePlugin('flexdiscount', 'shop');
    ...
    public static function getProductDiscounts($product, $view_type = null, $sku_id = 0, $return_html = true)
    {
        ...
        if (shopDiscounts::isEnabled('flexdiscount') && !empty($product)) {
            ...
            $html .= "<div class='flexdiscount-product-discount product-id-" . $product['id'] . (!$sku_id ? " f-update-sku" : "") . "' data-product-id='" . $product['id'] . "' data-view-type='" . ($view_type ? shopFlexdiscountHelper::escape($view_type) : '1') . "'";
            ...
            if (isset($workflows[$sku_id])) {
                ...
                $workflows[$sku_id] = shopFlexdiscountHelper::prepareProductWorkflow($workflow, $sku_id, wa('shop')->getConfig()->getCurrency(false), $product, $order_params);
                $workflow = $workflows[$sku_id];
                if (!isset($cache_work)) {
                    $cache_work = new waRuntimeCache('flexdiscount_product_workflows');
                }
    ...
    public static function getAvailableDiscounts($product = null, $view_type = null, $sku_id = 0, $filter_by = array(), $return_html = true)
    {
        ...
        if (shopDiscounts::isEnabled('flexdiscount')) {
            ...
            if ($available_discount) {
                ...
                $data = shopFlexdiscountHelper::prepareDiscountRuleData('available', $available_discount, wa('shop')->getConfig()->getCurrency(false), array('product' => $product));
                // Сортируем правила
            ...
            $html .= "<div class='flexdiscount-available-discount product-id-" . $product['id'] . (!$sku_id ? " f-update-sku" : "") . "' data-product-id='" . $product['id'] . "' data-view-type='" . ($view_type ? shopFlexdiscountHelper::escape($view_type) : '') . "'";
    ...
    public static function getDenyRules($product = null, $view_type = null, $sku_id = 0, $return_html = true)
    {
        ...
        if (shopDiscounts::isEnabled('flexdiscount')) {
            ...
            $html .= "<div class='flexdiscount-deny-discount product-id-" . $product['id'] . (!$sku_id ? " f-update-sku" : "") . "' data-product-id='" . $product['id'] . "' data-view-type='" . ($view_type ? shopFlexdiscountHelper::escape($view_type) : '') . "'";
            $html .= " data-sku-id='" . $product['sku_id'] . "'>";
    ...
    public static function price($product, $sku_id = 0, $view_type = 0)
    {
        ...
        if (shopDiscounts::isEnabled('flexdiscount') && !empty($product)) {
            ...
            if ($workflow['discount']) {
                ...
                $price_without_discount = $template_ruble_sign ? shop_currency_html($workflow['product']['price'], $workflow['product']['currency']) : shop_currency($workflow['product']['price'], $workflow['product']['currency']);
    ...
    public static function cartPrice($product, $mult_quantity = true, $html = null, $format = true, $update = true)
    {
        ...
        if (!empty($product)) {
            $price = $product_price = shop_currency($product['price'], $product['currency'], null, false);
            ...
            if (shopDiscounts::isEnabled('flexdiscount')) {
                ...
                if (!isset($cart_products[$product['sku_id']])) {
                    ...
                    if ($workflow['discount']) {
                        ...
                        if (!empty($workflow['products'][$product['sku_id']])) {
                            $workflow_product = $workflow['products'][$product['sku_id']];
            ...
            $price = $mult_quantity ? ($product_price * $quantity - $total_discount) : $price;
            $price = $format ? ($html ? shop_currency_html($price, true) : shop_currency($price, true)) : shop_currency($price, true, null, false);
    ...
    public static function getActiveCoupons($order_id = 0)
    {
        $coupons = array();
        if (shopDiscounts::isEnabled('flexdiscount')) {
            $workflow = shopFlexdiscountData::getOrderCalculateDiscount($order_id);
    ...
    {
        // TODO сделать инструкцию для вывода бонусов в уведомлениях
    ...
    public static function prepareProducts($params)
    {
        ...
        if (!empty($params['skus'])) {
            $data_class->setShopProductSkus($params['skus']);
        }
    }
