...
class shopFlexdiscountTypeHTMLBuilder extends shopFlexdiscountConditions
{
    ...
    private static function buildTargetDetailsHTML($target)
    {
        ...
        if (!empty($target['details']['field']) && !empty(self::$details[$target['details']['field']])) {
            ...
            $value = $field !== 'multiple' ? shopFlexdiscountHelper::escape($target['details']['value']) : $target['details']['value'];
            $html .= _wp(" to") . " ";
            if ($field == 'every') {
            ...
            } elseif ($field == 'ncheapest') {
                $html .= _wp("every") . " <input name='details' value='" . $value . "' type='text' onkeypress='$.flexdiscount.isValid(event, /[0-9]/);' class='width50px'> " . _wp('th cheapest products');
            ...
            } else if ($field == 'nexpensive') {
                $html .= _wp("every") . "<input name='details' type='text' value='" . $value . "' onkeypress='$.flexdiscount.isValid(event, /[0-9]/);' class='width50px'> " . _wp('th the most expensive products');
                ...
                $html .= " <input name='details' value='" . shopFlexdiscountHelper::escape($value[0]) . "' type='text' onkeypress='$.flexdiscount.isValid(event, /[0-9]/);' class='width50px'> " . _wp('products from') . " <input name='details2' value='" . shopFlexdiscountHelper::escape($value[1]) . "' type='text' onkeypress='$.flexdiscount.isValid(event, /[0-9]/);' class='width50px'> " . _wp("similar");
            }
    ...
    protected static function getProductFieldHTML($params, $condition)
    {
        ...
        if ($product_type == 'sku' && isset(self::$types_data['product'][$product_type][$condition[$params['name']]])) {
            ...
            $product = shopFlexdiscountHelper::escape($product) . ' (' . ($pdata['sku_name'] ? shopFlexdiscountHelper::escape($pdata['sku_name']) : ($pdata['sku'] ? shopFlexdiscountHelper::escape($pdata['sku']) : (!$pdata['sku_name'] && !$pdata['sku'] ? _wp('sku ID') . ': #' . $pdata['id'] : ''))) . ')';
        }
    ...
    protected static function getStocksFieldHTML($params, $condition)
    {
        ...
        if (!empty(self::$types_data['stocks'])) {
            ...
            foreach (self::$types_data['stocks'] as $o) {
                $html .= "<option value='" . $o['id'] . "'" . ($default === $o['id'] ? " selected" : "") . (!empty($o['class']) ? ' class="' . $o['class'] . '"' : '') . ">" . shopFlexdiscountHelper::escape($o['name']) . "</option>";
            }
        }
        $html .= "</select>";
        return $html;
    ...
    protected static function getTooltipFieldHTML($params, $condition)
    {
        ...
        $html .= '<div class="hover-tooltip">
                    ...
                    <div class="hover-content">
                        <div>
                            '.shopFlexdiscountHelper::escape($params['value']).'
                        </div>
                    </div>
                </div>';
        $html .= '</div>';
