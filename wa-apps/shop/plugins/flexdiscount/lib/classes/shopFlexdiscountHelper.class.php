...
class shopFlexdiscountHelper extends shopFlexdiscountData
{
    ...
    public static function workupProducts(&$products)
    {
        ...
        $currencyr = self::getOrderInfo('currency');
...
//                $p['price'] = shopRounding::roundCurrency($p['price'], $currencyr);
                $p['price_workuped'] = 1;
            }
...
//                    $p['purchase_price'] = shopRounding::roundCurrency($p['purchase_price'], $currencyr);
...
//                    $p['compare_price'] = shopRounding::roundCurrency($p['compare_price'], $currencyr);
            ...
            if ($skus !== null) {
                ...
                if (
                    (!isset($p['flexdiscount_minimal_discount_price']) || (float) $p['flexdiscount_minimal_discount_price'] === 0.0)
                    && isset($skus[$p['sku_id']]['flexdiscount_minimal_discount_price'])
                ) {
    ...
    public static function getSelectOptionsHtml($options, $selected = '')
    {
        ...
        foreach ($options as $o) {
            $html .= "<option value='" . $o['id'] . "'" . ($selected == $o['id'] ? " selected" : "") . (!empty($o['class']) ? ' class="' . $o['class'] . '"' : '') . ">" . self::escape($o['name']) . "</option>";
        }
        return $html;
    }
    ...
    public static function getServicesHtml($data, $selected = '')
    {
        ...
        foreach ($data['services'] as $s) {
            $html .= "<option" . ($s['selectable'] ? ' class="selectable"' : "") . " value='" . $s['id'] . "'" . ($selected == $s['id'] ? " selected" : "") . ">" . self::escape($s['name']) . "</option>";
        }
        return $html;
    }
    ...
    public static function getServicesVariantsHtml($data, $selected = '')
    {
        ...
        foreach ($data['variants'] as $v) {
            if (!empty($data['services'][$v['service_id']]) && $data['services'][$v['service_id']]['selectable']) {
                $html .= "<option class='feature-" . $v['service_id'] . "' value='" . $v['id'] . "'" . ($selected == $v['id'] ? " selected" : "") . ">" . self::escape($v['name']) . "</option>";
            }
        }
        return $html;
    }
    ...
    public static function getFeaturesHtml($features, $selected = '')
    {
        ...
        foreach ($features as $f) {
            ...
            $html .= "<option" . ($base_unit ? " data-base-unit = '" . $base_unit['title'] . "'" : "") . ' class="' . ($f['selectable'] ? 'selectable' : "") . ($base_unit ? ' dimension' : "") . '"' . " value='" . $f['id'] . "'" . ($selected == $f['id'] ? " selected" : "") . ">" . self::escape($f['name']) . ($f['code'] ? " (" . $f['code'] . ")" : "") . "</option>";
        }
        return $html;
    }
    ...
    public static function getFeaturesValuesHtml($values, $feature_id, $selected = '')
    {
        ...
        if (!empty($values)) {
            foreach ($values as $val_id => $val) {
                $html .= "<option class='feature-" . $feature_id . "' value='" . $feature_id . '-' . $val_id . "'" . ($selected == ($feature_id . '-' . $val_id) ? " selected" : "") . ">" . self::escape($val) . "</option>";
            }
        }
        return $html;
    }
    ...
    public static function getStorefrontRoutesHtml($routes, $selected = '')
    {
        ...
        foreach ($routes as $domain => $r) {
            foreach ($r as $route) {
                $html .= "<option class='domain-" . self::escape($domain) . "' value='" . self::escape($route) . "'" . ($selected == $route ? " selected" : "") . ">" . self::escape($route) . "</option>";
            }
        }
        return $html;
    }
    ...
    {
        $today = time();
        $order_params = (new shopFlexdiscountWorkflow())->getOrder();
     ...
     * @throws waException
     */
    public static function round($amount, $rounding = '', $type = 'discount')
    {
        static $round = null;
        if ($round === null) {
            $settings = self::getSettings();
            $round = array(
                'discount' => !empty($settings['round']) ? $settings['round'] : 'not',
                'affiliate' => !empty($settings['affiliate_round']) ? $settings['affiliate_round'] : 'not',
            );
        }
    ...
    /**
     * Get all discounts for frontend
     *
     * @return array
     */
    ...
    public static function prepareProductWorkflow($workflow, $sku_id, $currency, $product, $order_params)
    {
        ...
        if (!empty($workflow['products'][$sku_id])) {
            $workflow_data = $workflow['products'][$sku_id];
            ...
            if ($workflow_data['rules']) {
                $rules = self::prepareDiscountRuleData('pdiscount', $workflow_data['rules'], $currency, array("rules" => $workflow['active_rules']));
                ...
                $frontend_prices = self::getSettings('frontend_prices');
            ...
            $currency_rounding = self::getSettings('currency_rounding');
            ...
            if (!empty($currency_rounding)) {
                $product_price = shopRounding::roundCurrency(shop_currency($product_price, $product['currency'], $currency, false), $currency) - $product_discount;
            } else {
                $product_price = shop_currency($product_price, $product['currency'], $currency, false) - $product_discount;
            }
            ...
            $product_workflow = array(
                'discount' => $product_discount ? shop_currency($product_discount, $currency, $currency) : 0, // общая скидка для товара
                'discount_html' => $product_discount ? shop_currency_html($product_discount, $currency, $currency) : 0, // общая скидка для товара с символом рубля
                ...
                'price' => shop_currency($product_price, $currency, $currency), // цена товара со скидкой
                'price_html' => shop_currency_html($product_price, $currency, $currency), // цена товара со скидкой с символом рубля
                'clear_price' => $product_price, // чистая цена без валют
                'real_price' => shopRounding::roundCurrency(shop_currency($product['price'], $product['currency'], $currency, false), $currency),
                'product' => $product,
    ...
    public static function buildRuleHTMLCode($discount)
    {
        ...
        if (!empty($discount['description'])) {
            $html .= shopFlexdiscountHelper::escape($discount['description']) . ($discount['name'] ? ", " : "");
        }
        if (!empty($discount['name'])) {
            $html .= shopFlexdiscountHelper::escape($discount['name']);
        }
    ...
    public static function prepareDiscountRuleData($block_name, $discount_rule, $currency, $params)
    {
        ...
        foreach ($discount_rule as $k => $dr) {
            ...
            $data[$rule_id] = array(
                'name' => !empty($rule['name']) ? $rule['name'] : _wp('Discount #' . $rule_id),
                'description' => !empty($rule['description']) ? $rule['description'] : '',
                'sort' => isset($rule['sort']) ? $rule['sort'] : 0,
                'discount' => $dr['discount'] ? shop_currency($dr['discount'] / $dr['quantity'], $currency, null) : 0,
                'discount_html' => $dr['discount'] ? shop_currency_html($dr['discount'] / $dr['quantity'], $currency, null) : 0,
                ...
                'params' => array(
                    'discount' => !empty($rule_params['discount']) ? shop_currency($rule_params['discount'], $currency, null) : 0,
                    'discount_html' => !empty($rule_params['discount']) ? shop_currency_html($rule_params['discount'], $currency, null) : 0,
                    'discount_percentage' => !empty($rule_params['discount_percentage']) ? $rule_params['discount_percentage'] : 0,
                    'affiliate' => !empty($rule_params['affiliate']) ? $rule_params['affiliate'] : 0,
                    'affiliate_percentage' => !empty($rule_params['affiliate_percentage']) ? $rule_params['affiliate_percentage'] : 0,
                    'code' => !empty($rule_params['code']) ? $rule_params['code'] : '',
                )
            );
            if ($block_name == 'available') {
                $product_discount_price = $params['product']['price'] - ($dr['quantity'] ? $dr['discount'] / $dr['quantity'] : 0);
                $data[$rule_id]['price'] = shop_currency($product_discount_price, $currency, null);
                $data[$rule_id]['price_html'] = shop_currency_html($product_discount_price, $currency, null);
                $data[$rule_id]['clear_price'] = ($product_discount_price > 0) ? shop_currency($product_discount_price, $currency, null, 0) : 0;
                $data[$rule_id]['currency'] = $params['product']['currency'];
                $data[$rule_id]['sku_id'] = $params['product']['sku_id'];
            } else {
    ...
    public function getPluginSettingsPageUrl()
    {
        ...
        if (wa('shop')->getVersion() >= 8.5) {
            $url = wa()->getConfig()->getBackendUrl(true) . 'shop/marketing/discounts/flexdiscount/';
        }
        return $url;
    }
    ...
    public function getContact($check_zero_id = false)
    {
        $cache = new waRuntimeCache('igaponov_contact');
        if ($cache->isCached()) {
            $contact = $cache->get();
    ...
    public function getBackendAttention()
    {
        ...
        if ($attentions) {
            ...
            $plugin_url = wa()->getPlugin('flexdiscount')->getPluginStaticUrl();
            $result = <<<HTML
...
HTML;
    ...
    {
        return wa()->getAppPath('plugins/flexdiscount/templates/actions/' . $path);
    }
}
