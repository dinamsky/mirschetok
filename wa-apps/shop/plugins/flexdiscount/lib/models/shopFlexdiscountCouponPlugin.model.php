...
class shopFlexdiscountCouponPluginModel extends shopFlexdiscountPluginModelHelper
{
     ...
     * @throws waException
     */
    public static function couponCheck($order_id = 0)
    {
        static $coupons = null;
        $coupons = null;
        if ($coupons === null || waRequest::param('plugin', '') === 'quickorder') {
            $codes = (new shopFlexdiscountData())->getCouponCodes($order_id);
            if ($codes) {
                ...
                if ($coupons) {
                    ...
                    if ($coupons) {
                        // Получаем список всех правил скидок, у которых имеются данных купоны
