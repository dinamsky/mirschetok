    ...
    public function productAutocomplete($q)
    {
        ...
        if ($count < $this->limit) {
            ...
            if ($skus) {
                foreach ($skus as $s) {
                    $products[] = array(
                        'label' => shopFlexdiscountHelper::escape($s['name']) . ' <span class="hint">' . ($s['sku_name'] ? shopFlexdiscountHelper::escape($s['sku_name']) . ' ' : '') . ($s['sku'] ? ' (' . shopFlexdiscountHelper::escape($s['sku']) . ')' : '') . (!$s['sku_name'] && !$s['sku'] ? _wp('sku ID') . ': #' . $s['id'] : '') . '</span>',
                        'name' => shopFlexdiscountHelper::escape($s['name']),
                        'sku_name' => ($s['sku_name'] ? shopFlexdiscountHelper::escape($s['sku_name']) : ($s['sku'] ? shopFlexdiscountHelper::escape($s['sku']) : (!$s['sku_name'] && !$s['sku'] ? _wp('sku ID') . ': #' . $s['id'] : ''))),
                        'sku_id' => $s['id'],
                        'id' => $s['product_id']
                    );
                }
        ...
        foreach ($products as &$p) {
            $p['name'] = $p['label'] = shopFlexdiscountHelper::escape($p['name']);
        }
        ...
        if (waRequest::get('with_skus')) {
            ...
            foreach ($product_skus as $s) {
                $product_and_skus[] = array(
                    'label' => $products[$s['product_id']]['name'] . ' <span class="hint">' . ($s['name'] ? shopFlexdiscountHelper::escape($s['name']) . ' ' : '') . ($s['sku'] ? ' (' . shopFlexdiscountHelper::escape($s['sku']) . ')' : '') . (!$s['name'] && !$s['sku'] ? _wp('sku ID') . ': #' . $s['id'] : '') . '</span>',
                    'name' => $products[$s['product_id']]['name'],
                    'sku_name' => ($s['name'] ? shopFlexdiscountHelper::escape($s['name']) : ($s['sku'] ? shopFlexdiscountHelper::escape($s['sku']) : (!$s['name'] && !$s['sku'] ? _wp('sku ID') . ': #' . $s['id'] : ''))),
                    'sku_id' => $s['id'],
                    'id' => $s['product_id']
                );
            }
