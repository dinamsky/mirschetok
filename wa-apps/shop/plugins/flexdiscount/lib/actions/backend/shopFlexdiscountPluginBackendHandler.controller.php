...
class shopFlexdiscountPluginBackendHandlerController extends waJsonController
{
    ...
    public function execute()
    {
        ...
        if (!wa()->getUser()->isAdmin() && $method_name !== 'orderEditSaveCouponAction') {
            if (($method_name == 'editSettingsAction' && !wa()->getUser()->getRights("shop", "flexdiscount_settings")) || ($method_name !== 'editSettingsAction' && !wa()->getUser()->getRights("shop", "flexdiscount_rules"))) {
    ...
    private function editDiscountAction()
    {
        ...
        $percentage = shopFlexdiscountHelper::floatVal(waRequest::post("percentage", 0));
        $fixed_value = shopFlexdiscountHelper::floatVal(waRequest::post("fixed", 0));
    ...
    private function getProductsAction()
    {
        ...
        if ($this->response['products']) {
            ...
            foreach ($this->response['products'] as &$p) {
                ...
                $p['name_secure'] = shopFlexdiscountHelper::escape($p['name']);
    ...
    private function getProductSkusAction()
    {
        ...
        if ($product) {
            ...
            foreach ($skus as &$p) {
                ...
                $p['name_secure'] = shopFlexdiscountHelper::escape(shopFlexdiscountHelper::escape($product['name']));
                $p['sku_name'] = $p['name'];
                $p['sku_name_secure'] = $p['sku_name'] ? shopFlexdiscountHelper::escape($p['sku_name']) : ($p['sku'] ? shopFlexdiscountHelper::escape($p['sku']) : _wp('sku ID') . ': #' . $p['id']);
    ...
    private function getUsersAction()
    {
        ...
        if ($this->response['users']) {
            ...
            foreach ($this->response['users'] as &$u) {
                ...
                $u['name_secure'] = shopFlexdiscountHelper::escape($u['name']);
                if (isset($u['photo'])) {
    ...
    private function orderEditSaveCouponAction()
    {
        ...
        if ($order_id) {
            ...
            $this->response = shopFlexdiscountHelper::escape($code);
        }
    }
