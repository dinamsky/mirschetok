...
class shopQuickorderPluginProductData
{
    ...
    public function prepareProducts($products, $find_services = false, $skip_flexdiscount = false, $find_available = false)
    {
        ...
        if ($find_available && !wa('shop')->getConfig()->getGeneralSettings('ignore_stock_count')) {
        ...
        foreach ($products as $k => &$p) {
            ...
            if ($find_available && !wa('shop')->getConfig()->getGeneralSettings('ignore_stock_count') && (!$sku['available'] || ($sku['count'] <= 0 && $sku['count'] !== null)) && !empty($available_skus[$p['id']])) {
            ...
            $is_available = $product_status && $sku['available'] && (wa('shop')->getConfig()->getGeneralSettings('ignore_stock_count') || $sku['count'] === null || $sku['count'] > 0);
            if (!$is_available) {
                unset($products[$k]);
                continue;
            }
