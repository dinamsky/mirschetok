...
class shopQuickorderPlugin extends shopPlugin
{
    ...
    private static function getHead()
    {
        ...
        if (empty($settings['product']['hide_button']) || empty($settings['cart']['hide_button'])) {
            ...
            $head .= "<script type='text/javascript' src='" . wa()->getAppStaticUrl('shop') . "plugins/quickorder/js/dialog/jquery.dialog." . (!$is_debug ? 'min.' : '') . "js" . (!$is_debug ? "?v=" . time() : '') . "'></script>";
        }

        $helper = self::getClass('helper');

        $head .= "<script type='text/javascript' src='" . wa()->getAppStaticUrl('shop') . "plugins/quickorder/js/frontend." . (!$is_debug ? 'min.' : '') . "js" . (!$is_debug ? "?v=" . time() : '') . "'></script>";
