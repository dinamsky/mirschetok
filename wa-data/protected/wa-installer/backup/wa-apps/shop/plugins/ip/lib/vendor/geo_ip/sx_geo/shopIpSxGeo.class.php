...
class shopIpSxGeo {
	...
	public function __construct($db_file = 'SxGeo.dat', $type = self::SXGEO_FILE){
		...
		if(substr($header, 0, 3) != 'SxG') throw new waException("Can't open {$db_file}\n");
		...
		if($info['b_idx_len'] * $info['m_idx_len'] * $info['range'] * $info['db_items'] * $info['time'] * $info['id_len'] == 0) throw new waException("Wrong file format {$db_file}\n");
